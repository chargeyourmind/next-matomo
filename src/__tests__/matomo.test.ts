...
describe('excludeUrlsPatterns', () => {
  ...
  it('should excluded login.php and token variables', () => {
    ...
    Router.events.emit('routeChangeComplete', '/login')
    Router.events.emit('routeChangeComplete', '/path/to/page')
    Router.events.emit('routeChangeComplete', '/path/to/page?token=some-token-123')
    return new Promise<void>((resolve) => {
      setTimeout(() => {
        expect(window._paq).toMatchSnapshot()
        resolve()
